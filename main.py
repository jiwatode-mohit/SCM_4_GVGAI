...
def evaluate_with_variant_descriptions(
    ...
    Path("results_task_1").mkdir(exist_ok=True)
    out_base = _result_basename(model_name, job, quantization)
    out_path = Path("results_task_1") / f"{out_base}.json"
    ...
    for mode in modes:
        ...
        if analyze:
            metrics_path = Path("results_task_1") / f"{out_base}.{mode}.metrics.json"
        ...
        if emit_csv and rows is not None:
            csv_path = Path("results_task_1") / f"{out_base}.{mode}.predictions.csv"
...
def evaluate(
    ...
    structured results to ``results_task_1/<model>@<job>.json``.
    """
